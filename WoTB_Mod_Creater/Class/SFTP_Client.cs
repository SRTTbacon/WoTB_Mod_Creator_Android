...
namespace WoTB_Mod_Creater.Class
{
    ...
    public class SFTP_Client
    {
        ...
        public delegate void Upload_File_Length_Event<T>(T args);
        public event Upload_File_Length_Event<ulong> Upload_File_Length;
        public delegate void Download_File_Length_Event<T>(T args);
        public event Download_File_Length_Event<ulong> Download_File_Length;
        public delegate void Upload_File_End_Event();
        public event Upload_File_End_Event Upload_File_End;
        public ulong Uploaded_Size = 0;
        ...
        public List<string> GetFileLines(string From_File, bool IsErrorLogMode = false)
        {
            if (!IsConnected)
                return new List<string>();
            else if (!SFTP_Server.IsConnected)
            ...
            {
                List<string> Temp = new List<string>();
                StreamReader stream = GetFileRead(From_File);
        ...
        public StreamReader GetFileRead(string From_File, bool IsErrorLogMode = false)
        {
            ...
            try
            {
                return SFTP_Server.Exists(From_File) ? SFTP_Server.OpenText(From_File) : new StreamReader(new MemoryStream());
            }
            catch (Exception e)
            {
                if (IsErrorLogMode)
        ...
        public bool Directory_Move(string From_Dir, string To_Dir, bool IsOverWrite, bool IsErrorLogMode = false)
        {
            ...
            try
            {
                foreach (string File_Now in GetFiles(From_Dir, true, false))
                {
                    ...
                    _ = File_Move(File_Now, To_Dir + Next_Dir + Path.GetFileName(File_Now), IsOverWrite);
                }
                return true;
            }
            catch (Exception e)
        ...
        public bool Directory_Delete(string To_Dir, bool IsErrorLogMode = false)
        {
            ...
            try
            {
                ...
                foreach (SftpFile file in SFTP_Server.ListDirectory(To_Dir))
                {
                    ...
                    {
                        if (file.IsDirectory)
                            _ = Directory_Delete(file.FullName);
                        else
        ...
        public void AsyncUploadFile(string From_File, string To_File, bool IsUseEvent, bool IsErrorLogMode = false)
        {
            if (!IsConnected)
                return;
            else if (!SFTP_Server.IsConnected)
            ...
            try
            {
                ...
                Task task = Task.Run(() =>
                {
                    using (Stream fs = File.OpenRead(From_File))
                    {
                        SFTP_Server.UploadFile(fs, To_File, true, (ulong upsize) =>
                        {
                            if (IsUseEvent)
                                Upload_File_Length?.Invoke(upsize);
                            else
                                Uploaded_Size = upsize;
                            if (!IsUploading)
                                fs.Close();
                        });
                        if (IsUseEvent)
                            Upload_File_End?.Invoke();
                        else
                            IsUploaded = true;
                    }
                    IsUploading = false;
        ...
        public bool AsyncDownloadFile(string From_File, string To_File, bool IsErrorLogMode = false)
        {
            if (!IsConnected)
                return false;
            else if (!SFTP_Server.IsConnected)
            ...
            try
            {
                ...
                foreach (SftpFile file in files)
                {
                    if ((file.Name != ".") && (file.Name != ".."))
                    {
                        ...
                        if (file.IsDirectory)
                            _ = DownloadDirectory(sourceFilePath, destFilePath);
                        else
